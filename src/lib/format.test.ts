...
describe('resolveCaptureUrl', () => {
  it('uses the computed url from the list endpoint', () => {
    assert.equal(
      resolveCaptureUrl({ id: '1', schedule_id: 's', file_path: 'a/b.png', url: '/screenshots/a/b.png' }, BASE),
...
describe('describeCapture', () => {
  it('keeps a missing diff distinguishable from a zero diff', () => {
    ...
    const unchanged = describeCapture({ id: '2', schedule_id: 's', file_path: 'b.png', diff_percent: 0 }, BASE);
...
describe('toolError', () => {
  it('reports API failures as tool errors carrying the fix', () => {
    ...
    assert.match(String(result.content[0]!.text), /CAPTUREZE_API_KEY/);
  });
});
